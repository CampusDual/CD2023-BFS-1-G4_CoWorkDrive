...
@Component({
  selector: 'app-trip-done',
  templateUrl: './trip-done.component.html',
  styleUrls: ['./trip-done.component.css']
})
export class TripDoneComponent implements OnInit {
    ...
    adaptResultBubble(dataTripsStats: Array<any>) {
      ...
      private static configureAxisFormat(chartConf: ChartConfiguration, preferences: OChartPreferences, oTableOptions: OTableOptions): void {
        let formatCallback = TripDoneComponent.getAxisFormatCallback(preferences.selectedXAxisType, preferences.selectedXAxis, oTableOptions);
        if (formatCallback != undefined) {
          chartConf.xDataType = formatCallback;
        }
        formatCallback = TripDoneComponent.getAxisFormatCallback(preferences.selectedYAxisType, preferences.selectedYAxis[0], oTableOptions);
        if (formatCallback != undefined) {
